...
([`COPYING`](COPYING)). It holds under either reading: LGPL-3.0 is GPL-3.0
plus additional permissions, which section 7 of GPL-3.0 lets a redistributor
remove. File headers still govern their files: GTNHLib's `LICENSE.txt`
stays, and the LWJGL and ASM files keep their BSD notices
...
- **License texts.** The jar carries no license or notice file: the host
  ...
  text ([`COPYING`](COPYING)) and the LWJGL and ASM notices, whose BSD terms
